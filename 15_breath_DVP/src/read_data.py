...
def read_csv_with_time(filename):
    ...
    with open(filename, 'r', encoding='utf-8') as csv_data:
        ...
        if "time" in time:
            ...
            try:
                start_time.extend(list(map(int, time.split('_'))))  
            except:
                time = ""
            ...
            if "End Time" in line:
                ...
                if scale_count == 0:                
                    ...
                    if len(log_data) >= 8:
                        ...
                        r = int(log_data[6].strip(' []'))
                        if r == 255:
                            resp.append(-1)
                        else:
                            resp.append(r)

    return np.array(data), np.array(acc_x), np.array(acc_y), np.array(acc_z), np.array(status), np.array(HR),  np.array(timestamp),np.array(resp), start_time
